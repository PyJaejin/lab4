...
int main(int argc, char *argv[])
{
    ...
    int i, maxfd = 0, numbytes; char buf[100];
...
void *handle_clnt(int client_sock)
{
    ...
    char method[10];
    char msg[BUF_SIZE]; // 메시지 변수 char method[10];
...
{
    char protocol[] = "HTTP/1.1 400 BadRequest\r\n";
    char server[] = "Server:Netscape-Enterprise/6.0\r\n";
    char contenttype[] = "Content-Type:text/html\r\n";
    char html[] = "<html><head>BADConnection</head><body><H1>Bad Request</H1></body></html>\r\n";
    char end[]="\r\n";
    printf("send err\n");
    write(client_sock, protocol, strlen(protocol)); write(client_sock, server, strlen(server));

    write(client_sock, contenttype, strlen(contenttype));
    write(client_sock, end, strlen(end)); write(client_sock, html, strlen(html)); fflush(fdopen(client_sock, "w"));
}
void send_msg(int client_sock) // send to all
{
    ...
    char contenttype[] = "Content-Type:text/html\r\n";
    char html[] = "<html><head>HelloWorld</head><body><H1>Hello World</H1></body></html>\r\n";
    char end[]="\r\n";
