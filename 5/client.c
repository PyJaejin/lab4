...
int main(int argc, char **argv)
{
    ...
    strcpy(user_info.id, "Jaejin");
    strcpy(user_info.pw, "1234");
    ...
    // 주소 변환
    iRet = inet_pton(AF_INET, ip, &stAddr.sin_addr.s_addr);
    if(iRet == 0)
    {
    ...
    // 서버 포트(포트 문을 열어준다.)
    stAddr.sin_port = htons(atoi(argv[2])); // 포트 3000번 연다.:
    ...
    printf("길이는 %d입니다..\n", strlen((char*)&user_info));
    write(iDs, (void*)&user_info, 60);
